...
func (t *SmallbankChaincode) Prov(stub shim.ChaincodeStubInterface, reads, writes map[string][]byte) map[string][]string {
	dependency := make(map[string][]string)
	function, args := stub.GetFunctionAndParameters()
	if function == "send_payment" {
		destAccount := accountKey(args[1])
		srcAccount := accountKey(args[2])
		// If applicable, we can directly infer the relevant keys from txn args,
		//   instead of read and writes provided from this function parameters
		// We only need to make sure that the dependent keys(the returned map keys) appear in the writeset
		// and keys being dependent in the readset.
		...
		// other functions involve a single account only and do not involve any provenance-related relationship.
		// do nothing
	}
	fmt.Printf("PROV Function [%s], args [%v], dep: [%v]\n", function, args, dependency)
	return dependency
}
