...
var logger = flogging.MustGetLogger("snapshotbasedtxmgr")
...
func NewSnapshotBasedTxMgr(ledgerid string, db privacyenabledstate.DB, stateListeners []ledger.StateListener,
	...
	db.Open()
	txmgr := &SnapshotBasedTxMgr{ledgerid: ledgerid, db: db, stateListeners: stateListeners}
...
func (txmgr *SnapshotBasedTxMgr) NewQueryExecutor(txid string) (ledger.QueryExecutor, error) {
	qe := newSnapshotQueryExecutor(txmgr, txid)
	return qe, nil
}
...
func (txmgr *SnapshotBasedTxMgr) NewTxSimulator(txid string) (ledger.TxSimulator, error) {
	...
	s, err := newSnapshotTxSimulator(txmgr, txid)
	if err != nil {
		return nil, err
	}
	return s, nil
...
func (txmgr *SnapshotBasedTxMgr) ValidateAndPrepare(blockAndPvtdata *ledger.BlockAndPvtData, doMVCCValidation bool) error {
...
func (txmgr *SnapshotBasedTxMgr) invokeNamespaceListeners() error {
...
func (txmgr *SnapshotBasedTxMgr) clearCache() {
...
func (txmgr *SnapshotBasedTxMgr) updateStateListeners() {
	for _, l := range txmgr.current.listeners {
		l.StateCommitDone(txmgr.ledgerid)
	}
}

func (txmgr *SnapshotBasedTxMgr) reset() {
	txmgr.current = nil
}
